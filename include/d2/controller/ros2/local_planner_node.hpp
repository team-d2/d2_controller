  ...
  : rclcpp::Node(node_name, node_namespace, options),
    ...
    global_plan_sub_(
      this->create_subscription<PathMsg>(
        "global_plan", 10, [this](PathMsg::SharedPtr msg) {
          this->update_global_plan(std::move(msg));
        })),
    pose_sub_(
      this->create_subscription<PoseMsg>(
        "pose", 10, [this](PoseMsg::SharedPtr msg) {
...
private:
  ...
  {
    // get frame_id
    auto frame_id = global_plan_msg->header.frame_id.empty() ? default_frame_id_ : global_plan_msg->header.frame_id;
    ...
    for (const auto & pose_msg_data : global_plan_msg->poses) {
      ...
      try {
        tf_msg = tf_buffer_.lookupTransform(
          section_frame_id, frame_id_, rclcpp::Time(0));
      } catch (const tf2::TransformException & ex) {
        RCLCPP_ERROR(this->get_logger(), "Failed to get transform: %s", ex.what());
        return;
      }
  ...
  void update_pose(const PoseMsg::SharedPtr & pose_msg)
  {
    ...
    try {
      tf_msg = tf_buffer_.lookupTransform(
        pose_msg->header.frame_id, frame_id_, rclcpp::Time(0));
    } catch (const tf2::TransformException & ex) {
      RCLCPP_ERROR(this->get_logger(), "Failed to get transform: %s", ex.what());
      return;
    }
    ...
    pose.stamp_nanosec = static_cast<std::uint64_t>(pose_msg->header.stamp.sec * 1e9) + pose_msg->header.stamp.nanosec;
